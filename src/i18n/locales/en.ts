export default {
  tabs: {
    workshop: "Workshop",
    wallpapers: "Installed",
    collections: "Collections",
  },
  buttons: {
    ok: "OK",
    open_workshop: "Browser",
    install: "Install wallpaper",
    yes: "Yes",
    no: "No",
    open_data_folder: "Open Data folder",
    check_updates: "Check for updates",
    download_release: "Download & Release Notes",
    changelog: "Changelog",
    view_on_github: "View on GitHub",
    refresh: "Refresh",
    browse: "Browse",
    open: "Open",
    copy_id: "Copy ID",
    delete: "Delete",
    cancel: "Cancel",
    later: "Later",
    remove: "Remove",
    continue: "Continue",
    close: "Close",
    legal: "Terms & Legal",
    download: "Download",
    skip_version: "Skip this version",
    start_install: "Start install",
    github: "GitHub",
    retry: "Retry",
    faq: "FAQ",
    back: "Back",
    submit: "Submit",
  },
  tooltips: {
    open_folder: "Open folder",
    open_workshop: "Open in Steam Workshop",
    delete_wallpaper: "Delete wallpaper",
    extract_wallpaper: "Extract .pkg file",
    install_wallpaper: "Apply wallpaper",
    tasks: "Active downloads and extractions",
    settings: "Settings",
    multi_download: "Multi-Download",
    info: "Information",
    refresh: "Refresh Page",
    translate_description: "Translate description",
    show_original: "Show original",
    sort_asc: "Ascending",
    sort_desc: "Descending",
    cancel_task: "Cancel task",
    clear_history: "Clear history",
    collapse_sidebar: "Collapse Bar",
    expand_sidebar: "Expand Bar",
    theme: "Switch theme",
    init_metadata: "Initialize metadata for all installed wallpapers",
    cancel_init_metadata: "Cancel metadata initialization",
    select_multiple: "Select Multiple",
    open_we: "Open WE",
    load_playlist: "Load Playlist",
    load_profile: "Load Profile",
    create_playlist_we: "Create Playlist in WE",
    create_profile_we: "Create Profile in WE",
    restore_defaults: "Restore Defaults",
    fixate_changes: "Fixate Changes",
    previous_wallpaper: "Previous Wallpaper",
    stop: "Stop",
    play: "Play",
    pause: "Pause",
    next_wallpaper: "Next Wallpaper",
    unmute: "Unmute",
    mute: "Mute",
    show_icons: "Show Desktop Icons",
    hide_icons: "Hide Desktop Icons",
    media_controls: "Media Controls",
    preset_settings: "Preset Settings",
  },
  steam_status: {
    logging_in: "Signing Steam",
    signed_in: "Signed in",
    unknown_account: "Account unknown",
    error: "Sign-in failed",
    title: "Steam parser",
  },
  preset_settings: {
    rate: "Playback Rate",
    alignment: "Alignment",
    flip_horizontal: "Flip Horizontal",
    alignment_position: "Alignment Position",
    alignment_x: "X Offset",
    alignment_y: "Y Offset",
    alignment_z: "Zoom",
    volume: "Volume",
    audio_processing: "Audio Processing",
    scheme_color: "Scheme Color",
    enable_colors: "Enable Color Settings",
    brightness: "Brightness",
    contrast: "Contrast",
    saturation: "Saturation",
    hue: "Hue Shift",
    filter: "Filter",
    filter_amount: "Filter Amount",
    options: {
      center: "Center",
      left: "Left",
      right: "Right",
      top: "Top",
      bottom: "Bottom",
      free: "Free",
      none: "None",
      k23: "K23",
      adventure: "Adventure",
      coloration: "Coloration",
      simple_film: "Simple Film",
      blue_navy: "Blue Navy",
      "80s_action": "80s Action",
      desert: "Desert",
      desperado: "Desperado",
      dusk: "Dusk",
      honey: "Honey",
      sandy_sky: "Sandy Sky",
      slate: "Slate",
      western: "Western",
      setting_sun: "Setting Sun",
      tower: "Tower",
      amber: "Amber",
      aliens: "Aliens",
      daisy: "Daisy",
      emerald: "Emerald",
      back_sea: "Back Sea",
      beach: "Beach",
      studio: "Studio",
      wasteland: "Wasteland",
      gameboy: "Gameboy"
    }
  },
  metadata_init: {
    initializing: "Initializing Metadata",
    complete: "Complete",
    error: "Error",
    fetching: "Fetching metadata for installed wallpapers...",
    error_message: "Failed to initialize metadata",
    rate_limit_error: "Rate limit exceeded. Please try again in a few minutes.",
  },
  labels: {
    input_links: "Paste Links or IDs",
    detected_preview: "Detected Preview",
    waiting_for_input: "Waiting for input",
    wallpapers_filtered: "Showing {{filtered}} of {{total}} wallpapers",
    total_size: "Total size: {{size}}",
    size: "Size: {{size}}",
    installed: "Installed:",
    description: "Description:",
    no_description: "No description available.",
    no_tasks: "No active tasks",
    task_cancelled: "Cancelled by user",
    tasks_history: "History",
    page: "Page",
    of: "of {{total}}",
    clear: "Clear Filters",
    miscellaneous: "Miscellaneous:",
    genre: "Genre:",
    authors: "Authors:",
    rating: "Rating:",
    posted: "Posted: {{date}}",
    updated: "Updated: {{date}}",
    author: "Author: {{author}}",
    tags: "Tags:",
    showing_wallpapers: "Showing {{start}}-{{end}} of {{total}} wallpapers",
    no_wallpapers_found: "No wallpapers found",
    showing_collections: "Showing {{start}}-{{end}} of {{total}} collections",
    no_collections_found: "No collections found",
    loading_dots: "Loading...",
    loading: "Loading…",
    search_placeholder: "Search...",
    end_of_search: "End of search results.",
    translating: "Translating...",
    author_collections: "Collections",
    collections: "Collections:",
    items_count: "Items: {{count}}",
    back: "Back",
    metadata_initialized: "Metadata initialized for {{count}} wallpapers",
    remove_account_question: "Remove account {{user}}?",
    collection_badge: "Collection",
    downloading: "Downloading…",
    deleting: "Deleting…",
    detected_ids: "{{count}} IDs detected",
    started_n_downloads: "Started {{count}} download(s)",
    current_version: "Current",
    latest_version: "Latest",
    update_available_for: "Update available: {{version}}",
    up_to_date: "Up to date",
    wallpapers: "Wallpapers",
    remove_account: "Remove account",
    collection_info: "Collection info",
    install: "Install",
    details: "Details",
    clear_selection: "Clear",
    select_all: "Select All",
    selected_count: "{{count}} selected",
    extract_selected: "Extract",
    delete_selected: "Delete",
    global_profiles: "Global Profiles",
    playlist_name: "Playlist Name",
    profile_name: "Profile Name",
    load_playlist: "Load Playlist",
    download_prefix: "Download",
    extract_prefix: "Extract",
    go_to_preset: "Go to Preset",
    preset_settings: "Preset Settings",
    fixate_changes: "Fixate Changes",
    project_properties: "Project Properties",
    global_settings: "Global Settings",
  },
  messages: {
    download_started: "Download started!",
    extraction_started: "Extraction started!",
    wallpaper_deleted: "Wallpaper deleted!",
    id_copied: "ID copied to clipboard!",
    cannot_delete_active_single:
      "This wallpaper is currently active in Wallpaper Engine. Please switch to a different wallpaper first, then try again.",
    confirm_delete:
      "Delete this wallpaper?\n\nThe wallpaper folder will be removed from your Wallpaper Engine library permanently. This action cannot be undone.",
    no_pkg_file: "No .pkg file found.",
    invalid_input: "No valid Workshop IDs or links found.",
    batch_input_placeholder:
      "Enter Workshop IDs and/or URLs\n(space or newline separated):",
    invalid_we_directory:
      "Invalid Wallpaper Engine directory.\nPlease select the correct folder.",
    error: "Error",
    translation_error: "Translation failed",
    changelog_error: "Failed to load changelog: {{error}}",
    no_releases: "No releases published yet.",
    signed_in_to_steam: "Signed in to Steam",
    removed: "Removed",
    applied: "Applied",
    apply_failed: "Apply failed",
    deleted: "Deleted",
    copied: "Copied",
    wallpaper_applied: "Wallpaper applied",
    delete_failed: "Delete failed",
    confirm_bulk_delete:
      "Delete {{count}} wallpaper(s)?\n\nThis action cannot be undone.",
    bulk_delete_with_active:
      "{{count}} of the selected wallpapers are currently active.\n\nPlease switch to a different wallpaper first, then try again.",
    bulk_extract_success: "Successfully extracted {{count}} wallpaper(s)",
    bulk_delete_success: "Successfully deleted {{count}} wallpaper(s)",
  },
  dialog: {
    tasks: "Tasks",
    about: "About",
    legal: "Terms & Legal",
  },
  settings: {
    interface_personalization: "Interface & Personalization",
    behavior_workflow: "Behavior & Workflow",
    system_integration: "System & Integration",
    general: "General",
    hotkeys: "Hotkeys",
    hotkeys_description:
      "Click a binding, then press a key or mouse side button to reassign. Press Escape to cancel. MouseBack/MouseForward refer to the side buttons on your mouse.",
    hotkeys_reset: "Reset All",
    hotkeys_group_pagination: "Pagination",
    hotkeys_group_navigation: "Navigation",
    hotkeys_group_general: "General",
    hotkeys_press_key: "Press a key…",
    hotkeys_click_to_change: "Click to change binding",
    hotkeys_press_key_for: "Press a key or mouse button for: {{label}}",
    hotkeys_reset_to_default: "Reset to default",
    hotkeys_action_page_prev: "Previous Page",
    hotkeys_action_page_next: "Next Page",
    hotkeys_action_page_first: "First Page",
    hotkeys_action_page_last: "Last Page",
    hotkeys_action_nav_workshop: "Go to Workshop",
    hotkeys_action_nav_collections: "Go to Collections",
    hotkeys_action_nav_installed: "Go to Installed",
    hotkeys_action_refresh: "Refresh",
    hotkeys_action_open_settings: "Open Settings",
    hotkeys_action_toggle_sidebar: "Toggle Sidebar",
    hotkeys_action_theme_cycle: "Cycle Theme",
    hotkeys_action_open_tasks: "Open Tasks",
    hotkeys_action_open_multi_download: "Open Multi-Download",
    appearance: "Appearance",
    language: "Language",
    language_hint: "Changes apply immediately",
    theme: "Theme",
    theme_hint: "Visual appearance of the interface",
    accent_color: "Accent color",
    accent_hint: "Accent color for highlights and buttons",
    enable_3d_cards: "3D Card effect",
    enable_3d_cards_hint: "Tilt cards on hover",
    show_active_filters: "Show active filters bar",
    show_active_filters_hint: "Display applied filter chips under the search bar",
    application: "Application",
    save_window_state: "Save Window State",
    save_window_state_hint: "Remembers position and size between sessions",
    auto_check_updates: "Auto Check Updates",
    auto_check_updates_hint: "Check for new versions on startup",
    minimize_on_apply: "Minimize on apply",
    minimize_on_apply_hint: "Window minimizes after wallpaper is applied",
    workshop: "Workshop",
    preload_next_page: "Preload Next Page",
    preload_next_page_hint: "Background loading of next page",
    wallpapers: "Wallpapers",
    auto_apply_last_downloaded: "Auto Apply Last Downloaded",
    auto_apply_hint: "Automatically set last downloaded wallpaper",
    layout_animations: "Smooth Grid Animations",
    layout_animations_hint: "Enable beautiful layout animations when resizing or sorting grids",
    layout_animations_warning: "This setting strongly affects performance and is not recommended on weak systems.",
    wallpaper_engine: "Wallpaper Engine",
    we_directory: "WE Directory",
    we_directory_hint: "Path to Wallpaper Engine installation",
    account: "Account",
    steam_session: "Steam web session",
    steam_session_description:
      "Log in to Steam for the parser to view 18+ content.",
    download_account: "Download account",
    download_account_description:
      "Pick a Steam account used for downloading.",
    infinite_retry_accounts: "Endless download attempts",
    infinite_retry_accounts_hint: "Iterate through accounts to upload endlessly in a loop (or reconnect to a custom one)",
    add_custom_account: "Add custom account",
    add_custom_account_title: "Steam download account",
    add_custom_account_description:
      "Enter the Steam credentials.",
    custom_badge: "custom",
    auto_account: "Auto",
    signed_in: "Signed in",
    not_signed_in: "Not signed in",
    waiting_for_login: "Waiting for login…",
    open_parser: "Open Parser",
    relogin: "Relogin to system account",
    login_button: "Login",
    login_title: "Steam Login",
    login_description: "Enter your Steam account credentials to log in to the parser.",
    login_2fa_description: "Enter your Steam Guard code or confirm the login on your mobile device.",
    steam_guard_help: "If Steam Guard mobile confirmation is requested, just confirm it on your phone and wait.",
    steam_guard_code: "Code from mobile app or email",
    dont_ask_again: "Do not ask again",
    continue_offline: "Continue anonymously (dont ask again)",
    auto_open_login: "Automatically open login form on failure",
    auto_open_login_hint: "Show the login form if Steam session requires authentication on startup",
    login: "Login",
    parser_logs: "Parser Logs",
    parser_log: "Parser Logs",
    steam_qr_help: "Scan with the Steam Mobile App to sign in",
    steam_account_unknown: "Unknown",
    custom_accounts: "Custom accounts",
    custom_accounts_description: "Encrypted on disk with a machine-bound key.",
    username: "Username",
    password: "Password",
    add_account_button: "Add",
    no_custom_accounts: "No custom accounts yet.",
    remove_account: "Remove account",
    account_added: "Account added",
    account_exists: "This account already exists",
    confirm_remove_account: "Remove this account?",
    initialize_now: "Initialize Now",
    login_qr: "Login with QR",
    login_qr_desc: "Scan this code with your Steam Mobile App to sign in.",
    generating_qr: "Generating QR code…",
  },
  info: {
    description:
      "This application allows you to easily download and manage Wallpaper Engine wallpapers from Steam Workshop.",
    developed: "Developed with ❤ free",
    changelog_title: "Release notes",
    app_full_name: "WEave",
    version_label: "Version",
    faq_title: "Frequently Asked Questions",
  },
  debug: {
    entries: "Entries",
    no_entries: "No entries",
  },
  common: {
    refresh: "Refresh",
    clear: "Clear",
  },
  legal: {
    disclaimer_tab: "Disclaimer",
    license_tab: "License",
    no_affiliation_title: "No Affiliation",
    no_affiliation_text:
      "WEave is an independent project and is not affiliated with, endorsed by, or sponsored by Valve Corporation or Kristjan Skutta (creator of Wallpaper Engine).",
    use_at_risk_title: "Use at Your Own Risk",
    use_at_risk_text:
      'This software is provided "as is" without warranty of any kind. The developers are not responsible for any damage, data loss, or other issues that may arise from using this software.',
    steam_responsibility_title: "Steam Account Responsibility",
    steam_responsibility_text:
      "Users are solely responsible for complying with Steam's Terms of Service and Subscriber Agreement. WEave does not guarantee against Steam account restrictions or bans resulting from its use.",
    workshop_content_title: "Third-party Content",
    workshop_content_text:
      "WEave does not control, endorse, or assume responsibility for any content downloaded from the Steam Workshop. Users are solely responsible for the content they download and use.",
    third_party_notice_text: "For full third-party notices and component licenses, see",
    accept_terms: "I understand and accept these terms",
    accept_explanation:
      "By using WEave, you acknowledge that you have read and understood this disclaimer.",
  },
  filters: {
    local_sort: {
      install_date: "Install Date",
      name: "Name",
      rating: "Rating",
      size: "Size",
      posted_date: "Posted Date",
      updated_date: "Updated Date",
    },
    sort: {
      trend: "Popular",
      toprated: "Top Rated",
      mostrecent: "Most Recent",
      lastupdated: "Recently Updated",
      totaluniquesubscribers: "Most Subscribed",
    },
    time_period: {
      "1": "Today",
      "7": "This Week",
      "30": "This Month",
      "90": "3 Months",
      "180": "6 Months",
      "365": "This Year",
      "-1": "All Time",
    },
    category: {
      empty: "All",
      Wallpaper: "Wallpaper",
      Preset: "Preset",
      Asset: "Asset",
    },
    type: {
      empty: "Any",
      Scene: "Scene",
      Video: "Video",
      Application: "Application",
      Web: "Web",
    },
    age_rating: {
      empty: "Any",
      Everyone: "Everyone",
      Questionable: "Questionable",
      Mature: "Mature",
    },
    resolution: {
      empty: "Any",
      Standard_Definition: "SD",
      "1280_x_720": "720p",
      "1366_x_768": "768p",
      "1920_x_1080": "1080p",
      "2560_x_1440": "1440p",
      "3840_x_2160": "4K",
      Ultrawide_Standard_Definition: "UW SD",
      Ultrawide_2560_x_1080: "UW 1080p",
      Ultrawide_3440_x_1440: "UW 1440p",
      Dual_Standard_Definition: "Dual SD",
      Dual_3840_x_1080: "Dual 1080p",
      Dual_5120_x_1440: "Dual 1440p",
      Dual_7680_x_2160: "Dual 4K",
      Triple_Standard_Definition: "Triple SD",
      Triple_4096_x_768: "Triple 768p",
      Triple_5760_x_1080: "Triple 1080p",
      Triple_7680_x_1440: "Triple 1440p",
      Triple_11520_x_2160: "Triple 4K",
      Portrait_Standard_Definition: "Portrait SD",
      Portrait_720_x_1280: "Portrait 720p",
      Portrait_1080_x_1920: "Portrait 1080p",
      Portrait_1440_x_2560: "Portrait 1440p",
      Portrait_2160_x_3840: "Portrait 4K",
      Dynamic_resolution: "Dynamic",
      Other_resolution: "Other",
    },
    misc_tags: {
      Approved: "Approved",
      "Audio responsive": "Audio Responsive",
      "3D": "3D",
      Customizable: "Customizable",
      "Puppet Warp": "Puppet Warp",
      HDR: "HDR",
      "Media Integration": "Media Integration",
      "User Shortcut": "User Shortcut",
      "Video Texture": "Video Texture",
      "Asset Pack": "Asset Pack",
    },
    genre_tags: {
      Abstract: "Abstract",
      Animal: "Animal",
      Anime: "Anime",
      Cartoon: "Cartoon",
      CGI: "CGI",
      Cyberpunk: "Cyberpunk",
      Fantasy: "Fantasy",
      Game: "Game",
      Girls: "Girls",
      Guys: "Guys",
      Landscape: "Landscape",
      Medieval: "Medieval",
      Memes: "Memes",
      MMD: "MMD",
      Music: "Music",
      Nature: "Nature",
      "Pixel art": "Pixel Art",
      Relaxing: "Relaxing",
      Retro: "Retro",
      "Sci-Fi": "Sci-Fi",
      Sports: "Sports",
      Technology: "Technology",
      Television: "Television",
      Vehicle: "Vehicle",
      Unspecified: "Unspecified",
    },
    asset_type: {
      empty: "Any",
      Particle: "Particle",
      Image: "Image",
      Sound: "Sound",
      Model: "Model",
      Text: "Text",
      Sprite: "Sprite",
      Fullscreen: "Fullscreen",
      Composite: "Composite",
      Script: "Script",
      Effect: "Effect",
      "Scripted Layer": "Scripted Layer",
    },
    asset_genre: {
      empty: "Any",
      "Audio Visualizer": "Audio Visualizer",
      Background: "Background",
      Character: "Character",
      Clock: "Clock",
      Fire: "Fire",
      Interactive: "Interactive",
      Magic: "Magic",
      "Post Processing": "Post Processing",
      Smoke: "Smoke",
      Space: "Space",
    },
    script_type: {
      empty: "Any",
      Boolean: "Boolean",
      Number: "Number",
      Vec2: "Vec2",
      Vec3: "Vec3",
      Vec4: "Vec4",
      String: "String",
      "No Animation": "No Animation",
      Oversized: "Oversized",
    },
    workshop_filters: "Filters",
    search_options: "Search Options",
    search_fields: {
      title: "Specify which text fields you want to search:",
      title_and_description: "Title and description",
      title_only: "Title only",
      description_only: "Description only",
      help_text: "You can create more complex search queries using the operators AND, OR and NOT (use uppercase letters). You can also use the + and - characters as shorthand for AND and NOT.",
      help_example_and: "allows you to find items whose searchable text contains both \"red\" and \"blue\"",
      help_example_or: "allows you to find items whose searchable text contains \"red\" or \"blue\"",
      help_example_not: "allows you to find items whose searchable text contains \"red\", but not \"blue\"",
      grouping_text: "Grouping search criteria in parentheses allows you to create fairly precise queries.",
      grouping_example_1: "allows you to find items whose searchable text contains \"room\", as well as \"red\" or \"blue\".",
      grouping_example_2: "allows you to find items whose searchable text contains \"room\" and \"red\" or \"blue\", but not \"green\".",
    },
    time_period_title: "Time Period",
    reset_all: "Reset All",
    done: "Done",
    active_filters: "Active Filters",
    assets_and_scripts: "Assets & Scripts",
    incompatible_items: "Incompatible items",
    incompatible_desc: "Some items are not possible to include in the game. You can still find them if you select this box.",
    special_filters: "Special filters",
    date_filter: {
      tooltip: "Filter by Date",
      title: "Filter by Date",
      description: "Filter items by creation and update dates in Steam Workshop",
      time_created: "Time Created",
      time_updated: "Time Updated",
      from: "From",
      to: "To",
      clear: "Reset",
      apply: "Apply",
    },
  },
  collection_stats: {
    rating: "Rating",
    items: "Items",
    visitors: "Visitors",
    subscribers: "Subscribers",
    favorited: "Favorited",
    total: "Total",
    posted: "Posted",
    updated: "Updated",
  },
  tag_categories: {
    Genre: "Genre",
    Miscellaneous: "Miscellaneous",
    Misc: "Miscellaneous",
    Type: "Type",
    Category: "Category",
    Resolution: "Resolution",
    "Age Rating": "Age Rating",
    "Asset Type": "Asset Type",
    "Asset Genre": "Asset Genre",
    "Script Type": "Script Type",
    "Content Descriptors": "Content Descriptors",
    Tags: "Tags",
  },
  window: {
    minimize: "Minimize",
    maximize: "Maximize",
    restore: "Restore",
    close: "Close",
  },
  setup: {
    failed_check_logs: "Some installations failed. Please check logs.",
    checking: "Checking...",
    downloading: "Downloading...",
    installing: "Installing...",
    ready: "Ready",
    error: "Installation Error",
    preparing: "Preparing...",
  },
  bootstrap: {
    download_failed: "Download failed ({{id}}): {{status}}",
    download_cancelled: "Download cancelled: {{id}}",
    download_completed: "Download completed: {{id}}",
    extract_failed: "Extract failed ({{id}}): {{status}}",
    extract_completed: "Extract completed: {{id}}",
  },
  faq: {
    login_issues_q: "Steam login error occurs in the app",
    login_issues_a: "If you do not need 18+ content, you don't have to worry about this and can simply ignore it.\n\nIf you need 18+ content, try these options one by one:\n\n1) Simply restart the program\n2) Go to the settings and click \"Relogin to system account\"\n3) Open the data folder (the \"Open Data folder\" button in the \"About\" window) and delete the «SteamWebView» folder (after closing WEave) (this will reset the built-in browser cache). Restart the program.\n\nIf the issue persists for a long time, there are likely problems with the parser's system account. Try providing your own Steam account in the parser settings (Accounts -> Login). You can use any account (even a dummy one), as long as there are no restrictions on 18+ content in your account settings.\n\nIf necessary, more system accounts with automatic selection will be created in the future.",
    extract_q: "Why does the \"Extract\" button not work for some downloaded wallpapers?",
    extract_a: "Not all wallpapers in the Workshop are full-fledged projects (with a project.json file). There are video wallpapers (MP4) or web wallpapers (HTML). They are already ready to use, and there is no need (and technically impossible) to unpack them as Wallpaper Engine projects.",
    storage_q: "Where are downloaded wallpapers stored?",
    storage_a: "All downloaded wallpapers are saved directly to the root Wallpaper Engine folder in the \"projects/myprojects\" directory. They immediately become available in the official application without any additional actions on your part.",
    visibility_q: "Wallpapers are downloaded, but they are not visible in the original application",
    visibility_a: "Make sure that in the Settings you have specified the correct path to the Wallpaper Engine folder. The path must point to the root folder where the executable files (wallpaper32.exe / wallpaper64.exe) are located.",
    download_issues_q: "What to do if wallpapers won't download?",
    download_issues_a: "Try again, maybe wait a little bit.\nYou can also try to turn on the VPN.\n\nIt's possible that the passwords for the system download accounts have expired. In this case, you can:\n\n1) Add a friend's account or your own account with a copy of Wallpaper Engine in settings\n2) Buy an offline activation of Wallpaper Engine on various marketplaces and add it using the method above (They are very cheap)\n3) Write an Issue in the GitHub repository and I will try to promptly update the working accounts"
  },
} as const;
