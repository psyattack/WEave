...
### <strong>開発中 2.0 - Tauri (React) + Rust または PyWebView への移行。新鮮なUI、より速く、よりコンパクト。</strong>
...
> すべての依存関係はすでにアーカイブに含まれているので、アーカイブ удобное место выпрямите и `WEave.exe` を実行するだけです
...
- [ ] PyInstaller --onefileは再起動を壊すため、ソースからビルドする場合は--onedirでビルドしてください（~500mbなのでリリースには含めていません）
