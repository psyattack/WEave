...
from PyQt6.QtCore import Qt, QRect, QSize, QPropertyAnimation, QEasingCurve, QParallelAnimationGroup, QTimer
...
class FlowLayout(QLayout):
    ...
    def set_item_size_range(self, min_size: int, max_size: int, target: int):
    ...
    def _calculate_item_size(self, available_width: int) -> tuple:
    ...
    def _do_layout(self, rect: QRect, test_only: bool) -> int:
        ...
        if not test_only:
            should_animate = (
                self._animate and 
                self._cached_item_positions and 
                len(self._cached_item_positions) == len(new_positions) and
    ...
    def _apply_positions_immediate(self, positions: List[QRect]):
    ...
    def _resize_grid_item(self, widget: QWidget, new_size: int):
        if hasattr(widget, 'item_size'):
            ...
            if old_size != new_size:
                ...
                if hasattr(widget, 'name_label'):
                    ...
                    widget.name_label.setStyleSheet(f"""
                        color: white;
    ...
    def _animate_to_positions(self, new_positions: List[QRect]):
        self._stop_animations()
        ...
        for i, item in enumerate(visible_items):
            if i >= len(new_positions) or i >= len(self._cached_item_positions):
                continue
            ...
            old_rect = self._cached_item_positions[i]
            new_rect = new_positions[i]
...
class AdaptiveGridWidget(QWidget):
    ...
    def schedule_layout_update(self, delay_ms: int = 50):
